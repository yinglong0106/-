  ...
  <el-dialog title="查看月采购验收通过订单数量详情" :visible.sync="isOpen" width="700px" @close="beforeClose" :append-to-body="true">
    <div style="padding-bottom: 60px;">
      <el-descriptions :colon="false" class="margin-top" :column="1" :border="true" style="margin-bottom: 20px;">
        ...
        <el-descriptions-item label-class-name="my-label" content-class-name="my-content" label="采购验收通过订单数量">{{row.demandAcceptOkNum}}
        </el-descriptions-item>
      </el-descriptions>
      <el-table :data="tableData" border v-loading="loading">
        <el-table-column prop="day" label="日期" align="center"></el-table-column>
        <el-table-column prop="demandAcceptOkNum" label="验收通过订单数量" align="center"></el-table-column>
        <el-table-column prop="demandAcceptOkNum" label="操作" align="center">
    ...
    <checkOrder v-if="isRegister" @close="closeRegisterDetail" :row="row" :secRow="openRow"></checkOrder>
  </el-dialog>
</template>
...
<script>
  ...
  import {
    statisticGetMonthDeOrderOkNumByDirId
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        this.statisticGetMonthDeOrderOkNumByDirId()
      },
      async statisticGetMonthDeOrderOkNumByDirId() {
        this.loading = true
        statisticGetMonthDeOrderOkNumByDirId({
