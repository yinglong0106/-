...
import viewOrUpdateModel from '@/views/communication/userManagement/components/viewOrUpdateModel'
...
export default {
  ...
  methods: {
    async getList() {
      ...
      this.loading = true
      const user = getUser()
      let roleName = this.$route.name == 'ServiceCommunicationMsg'?'serveRole':'commuRole'
      if (user.roleNames.indexOf(roleName) > -1) {
        this.dataList = [user]
      } else {
        this.dataList = []
      }
