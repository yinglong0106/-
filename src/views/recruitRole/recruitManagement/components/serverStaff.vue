...
<script>
import { serverGroupList } from '@/api/recruitRoleApi/recruitManagement';
...
export default {
  ...
  methods: {
    async getList() {
      this.loading = true
      const res = await serverGroupList({ size: '20', page: this.page, phonenumber: this.searchVal, seorgGuid: this.currentRow.seorgGuid, curUserId: this.$store.state.user.adminId, })
      this.loading = false;
      if (res.Tag.length) {
