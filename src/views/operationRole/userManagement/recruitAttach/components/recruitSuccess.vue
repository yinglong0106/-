<template>
	<el-dialog title="招募专员成果" :visible.sync="isOpen" width="1000px" @close="beforeClose" append-to-body>
			...
			<el-tabs v-model="activeName" type="card" @tab-click="handleClick">
				<el-tab-pane label="采购对接成果" name="first">
					<div class="flex flex-center mb20">
						...
						<div class="ml0">展示有哪些服务机构关联过这个招募专员且统计为招募专员在采购对接这块带来多少业绩</div>
					</div>
					...
					<el-table :data="tableData" border v-loading="loading">
						...
						<el-table-column prop="createTime" label="账号创建日期" align="center" width="160"></el-table-column>
				...
				<el-tab-pane label="供应对接成果" name="second">
					<div class="flex flex-center mb20">
						<div class="title-bg1">采购订单验收通过数量</div>
						<div class="ml0">展示有哪些服务机构关联过这个招募专员且统计为招募专员在供应对接这块带来多少业绩</div>
					</div>
					<div class="flex jsb flex-center">
						<searchCom @toSearch='search' :searchResult='searchResult' placeholderText='请输入你要找的机构名称'>
						</searchCom>
...
<script>
	...
	import {
		getSeorgDeOrderNumByST3Uid,
		getSeorgSuOrderNumByST3Uid,
		...
		methods: {
			...
			getData() {
				if (this.activeName == 'first') {
					this.getSeorgDeOrderNumByST3Uid()
				} else if (this.activeName == 'second') {
					this.getSeorgSuOrderNumByST3Uid()
				}
			},
			async getSeorgDeOrderNumByST3Uid() {
				this.loading = true
				await getSeorgDeOrderNumByST3Uid({
			...
			async getSeorgSuOrderNumByST3Uid() {
				this.loading = true
				await getSeorgSuOrderNumByST3Uid({
					seorgName: this.searchVal,
					staffUserId: this.row.userId,
					size: '20',
					page: this.page,
