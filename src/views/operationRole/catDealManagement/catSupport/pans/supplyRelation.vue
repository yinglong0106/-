<template>
	<el-dialog title="查看供应组织关联关系" :visible.sync="isOpen" width="700px" @close="beforeClose" append-to-body>
		<div style="padding-bottom: 40px;" v-loading="loading">
			...
			<el-row class="mt20 flex flex-center">
				<div style="flex-shrink: 0;margin-right: 20px;width: 120px;">供应机构名称</div>
				<div>{{orgRow.orgName}}</div>
			</el-row>
			...
			<el-row class="mb20">
				<div style="background: #F2F2F2;padding: 10px;">供应专员信息</div>
			</el-row>
			<el-row class="mt20 flex flex-center">
			...
			<el-row class="mb20">
				<div style="background: #F2F2F2;padding: 10px;">服务机构信息</div>
			</el-row>
			<el-row class="mt20 flex flex-center">
				<div style="flex-shrink: 0;margin-right: 20px;width: 120px;">机构名称</div>
				<div>{{tableData.seorgName}}</div>
			</el-row>
			<el-row class="mt20 flex flex-center">
				<div style="flex-shrink: 0;margin-right: 20px;width: 120px;">账号开通日期</div>
				<div>{{tableData.seorgCreateTime}}</div>
			</el-row>
		</div>
	</el-dialog>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getOrgDetailByOrgUid() {
				this.loading = true
				await getOrgDetailByOrgUid({
					orgUserId: this.orgRow.orgUserId,
