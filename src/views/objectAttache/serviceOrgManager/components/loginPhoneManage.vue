<template>
	<el-dialog title="登录手机号管理" :visible.sync="isOpen" width="700px" @close="beforeClose">
		<div style="padding-bottom: 40px;" v-loading="loading">
			...
			<el-row class="mb20">
				<div style="background: #F2F2F2;padding: 10px;">【服务机构】通过【机构名称+登录手机号+手机号验证码】方式登录，系统直接给机构设置登录手机号。</div>
			</el-row>
			<el-row class="mb20">
...
<script>
	import newLoginPhone from '@/views/objectAttache/serviceOrgManager/components/newLoginPhone.vue'
	import updataLog from '@/views/objectAttache/serviceOrgManager/components/updataLog.vue'
	import {
		getCurPhone
