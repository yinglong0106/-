		...
		<div style="padding-bottom: 40px;" v-loading="loading">
			...
			<el-row class="mt20">
				...
				<el-upload ref="upload" class="avatar-uploader" action="" :show-file-list="false" :auto-upload="false"
					:on-change="preUploadImg" :http-request="uploadImg" accept=".jpeg,.png,.jpg,.bmp,.gif"
					:before-upload="checkFileType" :multiple="true" :on-success="handleAvatarSuccess">
	...
	export default {
		...
		methods: {
			...
			async uploadImg(item) {
				...
				let FilePath = 'aprc\\web\\seorg\\phoneimg'
				let data = base64File
				this.loading = true
