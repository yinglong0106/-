<template>
	<div class="app-container" v-if="loading">
          ...
          <template slot-scope="scope">
            <p>{{scope.row.plateFieldSource=="1"?'固化':'自建'}}</p>
				...
				<el-table-column prop="fieldMessage" label="字段内容管理" align="center">
					<template slot-scope="scope">
						<p v-if="scope.row.fieldContentSource=='3' || scope.row.fieldContentSource=='4'"
							class="el-icon-check">
						</p>
				...
				<el-table-column label="操作" align="center">
					<template slot-scope="scope">
						<el-row>
							<el-button @click="openContentFrom(scope.row,scope.$index)" type="text" size="small">内容来源设置
							</el-button>
						</el-row>
						<el-row>
							<el-button @click="openDemander(scope.row,scope.$index)" type="text" size="small">
								{{openRow.type=='demand' || openRow.catTreeCode=='demand'?'需方':'供方'}}操作设置</el-button>
						</el-row>
						<el-row>
							<el-button @click="openFieldMessage(scope.row,scope.$index)" type="text" size="small">字段内容管理
							</el-button>
						</el-row>
					</template>
				</el-table-column>
		...
		<!-- 需方操作设置 -->
		<userOperation v-if="isDemander" :type="openRow.catTreeCode" :categoryName="openRow.categoryName"
			:fieldObj="tableData[0]" @close="closeDemander" @refresh="getPlateFieldContents"
			:bizType="this.openRow.bizType"></userOperation>

		<!-- 字段内容管理 -->
		<fieldContentManagement v-if="isFieldMessage" :type="openRow.catTreeCode" :categoryName="openRow.categoryName"
			:categoryGuid="openRow.categoryGuid" :fieldObj="tableData[0]" @close="closeFieldMessage"
			@refresh="getPlateFieldContents" :bizType="openRow.bizType"></fieldContentManagement>
	</div>
</template>
